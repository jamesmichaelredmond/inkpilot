...
export function registerSvgUpdateElement(server: McpServer, context: McpServerContext) {
  server.tool(
    'svg_update_element',
    ...
    {
      id: z.string().describe('The element ID to update (use svg_list_elements to find IDs)'),
      attributes: z.record(z.string()).describe('Attributes to set (e.g. {"fill":"red","opacity":"0.5"})'),
    },
    async ({ id, attributes }) => {
      ...
      return {
        content: [{
          type: 'text',
          text: `Element "${id}" updated: ${Object.entries(attributes).map(([k, v]) => `${k}="${v}"`).join(', ')}.`,
        }],
      };
    },
  );
