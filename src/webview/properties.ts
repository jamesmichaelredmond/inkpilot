...
function renderEmpty() {
    panel.innerHTML =
        '<div class="props-empty">Select an element to edit properties</div>';
}
...
function renderProperties(canvas: Canvas) {
    ...
    // Position
    addNumberField("X", obj.left ?? 0, (v) => {
        obj.set("left", v);
        canvas.renderAll();
    });
    addNumberField("Y", obj.top ?? 0, (v) => {
        obj.set("top", v);
        canvas.renderAll();
    });
    ...
    // Rotation
    addNumberField("Rotation", obj.angle ?? 0, (v) => {
        obj.set("angle", v);
        canvas.renderAll();
    });
